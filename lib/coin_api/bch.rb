module CoinAPI
  class BCH < BaseAPI
    ...
    # bch address generated and also convert the cash address to legacy (old bch) address.
    def create_address!(account)
      @cash_address = json_rpc(:getnewaddress,[account]).fetch('result')
      @converted_address = Cashaddress.to_legacy(@cash_address)
      {
       address: @cash_address,
       legacy_address: @converted_address
      }
    end
    ...
    def coin_legacy_address(address)
