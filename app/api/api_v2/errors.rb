module APIv2
  ...
  class WithDrawError < Error
    def initialize(error_msg)
      super code: 2013, text: "WithDraw error - #{error_msg}", status: 405
    end
  end

  class WithdrawByTxidNotFoundError < Error
    def initialize(txid)
      super code: 2014, text: "Withdraw##txid=#{txid} doesn't exist.", status: 404
    end
  end

  class JWTAuthenticationError < Error
    def initialize(email)
      super code: 2015, text: "User(#{email}) is already logged somewhere else ", status: 401
    end
  end
end
