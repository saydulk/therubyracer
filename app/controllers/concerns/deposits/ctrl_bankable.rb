module Deposits
  module CtrlBankable
    ...
    def fetch
      @deposit_currency = params[:deposit][:currency]
      # @account = current_user.get_account(channel.currency)
      @account = current_user.get_account(@deposit_currency)
      @model = model_kls
      @fund_sources = current_user.fund_sources.with_currency(@deposit_currency)
      @assets = model_kls.where(member: current_user).order(:id).reverse_order.limit(10)
    end

    def deposit_params
      params[:deposit][:currency] = @deposit_currency
