...
private
  ...
  def do_submit(order)
    ...
    order.save!
    order.hold_account.lock_funds(order.locked, reason: Account::ORDER_SUBMIT, ref: order)
  end
  ...
  def do_cancel!(order)
    order.with_lock do
      ...
      order.hold_account.unlock_funds(order.locked, reason: Account::ORDER_CANCEL, ref: order)
