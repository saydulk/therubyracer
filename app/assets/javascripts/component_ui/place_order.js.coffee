@PlaceOrderUI = flight.component ->
  @attributes
    ...
    submitButton: ':submit'
    percentageAmount: 'a.calc_amount'

  @panelType = ->
    switch @$node.attr('id')
      when 'bid_entry' then 'bid'
      when 'market_bid_entry' then 'bid'
      when 'ask_entry' then 'ask'
      when 'market_ask_entry' then 'ask'
  ...
  @confirmDialogMsg = ->
    ...
    """
  @getMarketFee = (market_type) ->
    switch market_type
      when 'ask'
        return gon.market.ask.fee
      when 'bid'
        return gon.market.bid.fee

  @beforeSend = (event, jqXHR) ->
    if (parseFloat(@select('totalSel').val()) <= 0) || (parseFloat(@select('totalSel').val()) < parseFloat(@getMarketFee(@panelType())))
      @cleanMsg()
      ef_class = 'shake shake-constant hover-stop'
      @select('dangerSel').append(JST["templates/hint_order_warning"]({msg: 'Failed'})).show().addClass(ef_class).wait(1500).removeClass(ef_class)
      window.sfx_warning()
      @resetForm(event)
      return false
  ...
  @handleSuccess = (event, data) ->
    @cleanMsg()
    @select('successSel').append(JST["templates/hint_order_success"]({msg: data.message})).show().delay(5000).fadeOut()
    @resetForm(event)
    window.sfx_success()
    @enableSubmit()
  ...
  @handleError = (event, data) ->
    ...
    @select('dangerSel').append(JST["templates/hint_order_warning"]({msg: json.message})).show().delay(5000).fadeOut()
  ...
  @updateAvailable = (event, order) ->
    ...
    available = formatter.fix type, parseFloat(@getBalance().minus(order[@usedInput]))
  ...
  @calculateAmount = (event) ->
    if gon.user
      val = $(event['currentTarget']).attr('value')
      if val == '25%'
        total = @getBalance()/4.0
      else if val == '50%'
        total = @getBalance()/2.0
      else if val == '75%'
        total = (@getBalance()* 3.0)/4.0
      else if val == '100%'
        total = @getBalance()
      @select('totalSel').val(total.toFixed(6))

      price = if @select('priceSel').length == 0
                gon.ticker.last
              else
                @select('priceSel').val()

      if parseInt(price) <= 0 or isNaN(parseInt(price))
        return false
      else
        price = BigNumber(price)

      @select('volumeSel').val((total/price).toFixed(6))

      available = parseFloat(this.getBalance()) - parseFloat(total)
      @select('currentBalanceSel').text(available.toFixed(6))
  ...
  @after 'initialize', ->
    ...
    @on @select('percentageAmount'), 'click', @calculateAmount
