window.MarketTradesUI = flight.component ->
  ...
  @handleMarketTrades = (event, data) ->
    console.log("In market trade js ============", data.trades)
  ...
  @handleMyTrades = (event, data, notify=true) ->
    if data.trades.length > 0
      ...
      @select('myTableSelector').html("<tr><td colspan='3' class='text-center' style='background:#fafafa;'> You have no trades </td></tr>")
  ...
  @after 'initialize', ->
    ...
    new SimpleBar(document.getElementById('scroll_trade_market'), { autoHide: false })
