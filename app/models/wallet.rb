...
class Wallet < ActiveRecord::Base
  KIND = %w[hot].freeze
  GATEWAYS = %w[bitcoind bitcoincashd  geth rippled tronixd ].freeze
  ...
  scope :deposit,  -> { where(kind: :hot) }
  #scope :withdraw, -> { where.not(kind: :hot) }
  scope :withdraw, -> { where(kind: :hot) }
