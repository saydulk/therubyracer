class Order < ActiveRecord::Base
  ...
  ATTRIBUTES = %w(id at market kind price state state_text volume origin_volume origin_locked locked funds_received)

  belongs_to :member
  attr_accessor :total
  scope :done, -> { with_state(:done) }
  scope :active, -> { with_state(:wait).where(is_locked: false) }
  ...
  scope :pending, -> { with_state(:wait).joins(:member).where('members.is_fake = ? && orders.updated_at < ? ', false, (Time.now - 30.minutes)) }
  ...
  def market_name
    Market.find_by_id(currency)&.name
  end
  ...
  def for_orders_history
    results = {}
    ATTRIBUTES.each do |attr|
      results = results.merge ({"#{attr}" => self.send(attr)})
    end
    results = results.merge({average: avg_price})
    results
  end
