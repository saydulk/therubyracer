class AccountVersion < ActiveRecord::Base
  ...
  before_create :deduct_transaction_bonus_from_fee, if: proc {|k| k.modifiable_type.casecmp('trade') == 0 && %w(strike_sub strike_add).include?(k.reason)}
  ...
  def deduct_transaction_bonus_from_fee
    member = Member.find(member_id)
    return unless plan = Referral.active
    if Member.current == member && member.sponsor.present? && plan.level_no > 1 && plan.transaction_bonus
      bonus = (fee * plan.transaction_bonus) / 100
      self.fee = fee - bonus
      member.sponsor.get_account(currency)&.update_columns(balance: bonus)
    end
  end
end
