class Account < ActiveRecord::Base
  ...
  after_create :payment_address

  def payment_address
   latest_payment_address =  payment_addresses.last || payment_addresses.create!(currency: self.currency)
   return latest_payment_address
  end
  ...
  scope :sum_24, -> (currency) { where('created_at > ?', 24.hours.ago).with_currency(currency) }

  scope :locked_sum_24, -> (currency) { sum_24(currency).sum(:locked) }
  scope :balance_sum_24, -> (currency) { sum_24(currency).sum(:balance) }
  ...
  def as_json(options = {})
    super(options).merge({
      ...
      "legacy_address" => payment_addresses.empty? ? "" : payment_address.legacy_address,
      "name_text" => currency_obj.name_text,
      "default_withdraw_fund_source_id" => default_withdraw_fund_source_id,
      "payment_id" => payment_addresses.empty? ? "" : payment_address.payment_id
